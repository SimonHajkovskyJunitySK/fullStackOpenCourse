...
- Name: blog_app.spec.js >> Blog app >> When logged in >> and a blog exists >> the blog can be liked
- Location: tests\blog_app.spec.js:83:7

# Error details

```
Error: locator.click: Error: strict mode violation: locator('.blog').filter({ hasText: 'Testing with Playwright' }).getByRole('button', { name: 'view' }) resolved to 2 elements:
