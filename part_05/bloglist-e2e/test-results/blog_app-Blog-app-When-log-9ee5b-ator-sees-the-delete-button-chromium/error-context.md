...
- Name: blog_app.spec.js >> Blog app >> When logged in >> and a blog exists >> only the creator sees the delete button
- Location: tests\blog_app.spec.js:115:7

# Error details

```
Error: locator.click: Error: strict mode violation: locator('.blog').filter({ hasText: 'Testing with Playwright' }).getByRole('button', { name: 'view' }) resolved to 4 elements:
    ...
    4) <button>view</button> aka getByRole('button', { name: 'view' }).nth(5)
...
- generic [ref=e3]:
  ...
  - generic [ref=e5]: welcome Juraj Novák
  - paragraph [ref=e6]:
    - text: Juraj Novák logged in
